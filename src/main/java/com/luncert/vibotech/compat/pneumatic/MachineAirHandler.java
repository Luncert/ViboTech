...
public class MachineAirHandler extends BasicAirHandler implements IAirHandlerMachine {
    ...
    public void tick(BlockEntity be) {
        ...
        if (!world.isClientSide) {
            ...
            if (safetyLeakDir != null) {
                ...
                doOverpressureChecks(be, world, pos);
            }
            ...
            if (prevLeakDir != actualLeakDir || actualLeakDir != null && (world.getGameTime() & 0x1f) == 0) {
                ...
                AllPackets.getChannel().send(PacketDistributor.TRACKING_CHUNK.with(() -> world.getChunkAt(be.getBlockPos())),
                    new UpdatePressureBlockPacket(be, anyConnectedFace(), actualLeakDir, getAir()));
            }
        ...
        if (actualLeakDir != null && getAir() != 0) {
            handleAirLeak(be, actualLeakDir);
        }
    }
    ...
    private void handleAirLeak(BlockEntity ownerTE, Direction actualLeakDir) {
        ...
        if (!world.isClientSide) {
            if (getAir() > 0) {
                int leakedAmount = (int) (pressure * AIR_LEAK_FACTOR) + 20;
            ...
            if (getAir() > 0) {
                if (pressure > 1f || pressure > 0.5f && world.random.nextBoolean() || world.random.nextInt(3) == 0) {
                    world.addParticle(new AirParticleData(), pos.getX() + 0.5D + mx * 0.6, pos.getY() + 0.5D + my * 0.6, pos.getZ() + 0.5D + mz * 0.6,
                            mx * speed, my * speed, mz * speed);
                }
            } else if (getAir() < 0 && world.random.nextBoolean()) {
                world.addParticle(new AirParticleData(), pos.getX() + 0.5D + mx, pos.getY() + 0.5D + my, pos.getZ() + 0.5D + mz, mx * speed, my * speed, mz * speed);
            }
    ...
    private LazyOptional<IAirHandlerMachine> getCachedNeighbourAirHandler(BlockEntity be, Direction dir) {
        if (!connectedFaces.get(dir.get3DDataValue())) return LazyOptional.empty();
        return CapabilityUtils.getNeighbourCap(AllCapabilities.AIR_HANDLER_MACHINE_CAPABILITY, be, dir);
    }
    ...
    private void disperseAir(BlockEntity ownerTE) {
        ...
        List<Connection> neighbours = getConnectedAirHandlers(ownerTE, true);
